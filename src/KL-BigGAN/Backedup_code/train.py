...
def run(config):
    #Create Output log for progress log
    f=open('%s/%s/training_file.txt' % (config['samples_root'],config['experiment_name']),"a")
    ...
    GD = model.G_D(G, D, config['conditional'])  # check if labels are 0's if "unconditional"
    print(G)
    print(D)
    ...
    loaders = utils.get_data_loaders(config, **{**config, 'batch_size': D_batch_size,'start_itr': state_dict['itr']})
    ...
    # Loaders are loaded, prepare the training function (Reweighting function is here*)
    ...
    for epoch in range(state_dict['epoch'], config['num_epochs']):
        ...
        for i, (x, y, ratio) in enumerate(pbar):
            ...
            if config['ema']:
                G_ema.train()
            if config['D_fp16']:
                x, y, ratio = x.to(device).half(), y.to(device), ratio.to(device)
            else:
            ...
            if config['pbar'] == 'mine':
                ...
                f=open('%s/%s/training_file.txt' % (config['samples_root'],config['experiment_name']),"a")
                f.write(', '.join(['itr: %d' % state_dict['itr']]
                                + ['%s : %+4.3f' % (key, metrics[key])
                                    for key in metrics])+"\n")
                f.close()
        ...
        if (epoch >= config['start_eval']):
            ...
            train_fns.update_FID(G, D, G_ema, state_dict, config, FID, experiment_name, test_log, epoch)  # added epoch logging
