...
def split_bias_dataset(num_class=2,res=64,samples=100,data_instances=10000,debug=0):
    ...
    Parameters
    ----------
    num_class : TYPE, optional
        Number of classes. The default is 2.
    res : TYPE, optional
        Resolution of the data. The default is 64.
    samples : TYPE, optional
        number of samples in each data-image. The default is 100.
    data_instances : TYPE, optional
        number of datapoints in the total population. The default is 10000.
    debug : TYPE, optional
        Debug-mode. The default is 0.
...
def split_bias_ref_dataset(num_class=2,res=64,samples=100,data_instances=10000,debug=0):
    '''
    ...
    #For all class cofiguration , we will only have one minority dataset
    # for i in np.arange (0.1,1,0.1):
    unbias_perc=1.0/num_class
    
    perc_minority=unbias_perc
    perc_majority=(1-perc_minority)/(num_class-1)
    ...
    torch.save((bias_list,label_list),os.path.join(directory,("test_fairness_ref_data_%f_samples_%i")%(unbias_perc,data_instances)))
    return (bias_list,label_list)
...
def split_bias_dataset_sample(num_class=2,res=64,samples=100,data_instances=20,debug=0,perc_minority=0.5):
    ...
    num_class : TYPE, optional
        Number of classes. The default is 2.
    res : TYPE, optional
        resolution of the sampled data. The default is 64.
    samples : TYPE, optional
        Number of sample points inside a single data-image. The default is 100.
    data_instances : TYPE, optional
        DESCRIPTION. The default is 20.
    debug : TYPE, optional
        DESCRIPTION. The default is 0.
    perc_minority : TYPE, optional
